...
namespace Github_TimeMachine
{
    ...
    public class Class1 : IExternalCommand
    {
        ...
        public void addFilterToView(Document doc, View v, ParameterFilterElement f)
        {
            ICollection<ElementId> allFilters = v.GetFilters();//new FilteredElementCollector(doc).OfClass(typeof(ParameterFilterElement)).ToElements();
            ...
            for (var i = 0; i < allFilters.Count(); i++)
            {
                ...
                if (pfe != null && pfe.Name.Contains("Filtromatisation"))
                {
                    filtersToBeDeletedIds.Add(pfe.Id);
                }
            }

            //Transaction deletingFiltersTransaction = new Transaction(doc, "Deleting filters");
            //deletingFiltersTransaction.Start();
            doc.Delete(filtersToBeDeletedIds);
            //deletingFiltersTransaction.Commit();
        }
            ...
            string name, List<ElementId> catids)
        {
            ...
            //lev1AFilter.Add(new ElementParameterFilter(new FilterIntegerRule(d1, new FilterNumericGreater(), (debutPhase))));
        ...
        #region Add filters
        public void addFiltersToSheets(Document doc, ICollection<ElementId> sheets, List<ElementId> catids)
        {
            ...
            int l_Demoli_surfaceTransp = -1;//null;//#100
            ...
            //ICollection<ElementId> sheets = new FilteredElementCollector(doc).OfClass(typeof(ViewSheet)).ToElementIds();
            int debutPhase, finPhase;
            foreach (ElementId sId in sheets)
            {
                ViewSheet s = doc.GetElement(sId) as ViewSheet;
                //if (s.GetParameters("GLS-PHA_Désignation")[0].AsString() == "MotomatisationFiltres")
                //{
                #region Deleting old filters
                //Transaction t = new Transaction(doc, "Creating filters");
                //t.Start();
                if (s.GetAllPlacedViews().Count != 0)
                {
                    View viewOnSheet = doc.GetElement(s.GetAllPlacedViews().FirstOrDefault()) as View;
                    ICollection<ElementId> allFilters = viewOnSheet.GetFilters();
                    Element pfe; // ParameterFilterElement
                    List<ElementId> filtersToBeDeletedIds = new List<ElementId>(); ;
                    ...
                    for (var i = 0; i < allFilters.Count(); i++)
                    {
                        pfe = doc.GetElement(allFilters.ElementAt(i));
                        if (pfe != null && pfe.Name.Contains("Filtromatisation")
                            && pfe.Name.Contains(s.Name)
                            && pfe.Name.Contains(s.SheetNumber))
                        {
                            filtersToBeDeletedIds.Add(pfe.Id);
                        }
                    }
                ...
                #region creating filters
                debutPhase = s.GetParameters("GLS-PHA_Construction-début")[0].AsInteger();
                finPhase = s.GetParameters("GLS-PHA_Construction-fin")[0].AsInteger();

                //t.Start();
                ParameterFilterElement f0 = f_EnDemolition(doc, debutPhase, finPhase, c0, c1, d0, d1, "GLS_PHA_" + debutPhase.ToString() + "_" + finPhase.ToString() + "_EnDemolition _" + s.Name + s.SheetNumber + "_Filtromatisation", catids);
                ParameterFilterElement f2 = f_EnConstruction(doc, debutPhase, finPhase, c0, c1, d0, d1, "GLS_PHA_" + debutPhase.ToString() + "_" + finPhase.ToString() + "_EnConstruction_" + s.Name + s.SheetNumber + "_Filtromatisation", catids);
                ParameterFilterElement f3 = f_DejaConstruit(doc, debutPhase, finPhase, c0, c1, d0, d1, "GLS_PHA_" + debutPhase.ToString() + "_" + finPhase.ToString() + "_DejaConstruit_" + s.Name + s.SheetNumber + "_Filtromatisation", catids);
                ParameterFilterElement f4 = f_DejaDemoli(doc, debutPhase, finPhase, c0, c1, d0, d1, "GLS_PHA_" + debutPhase.ToString() + "_" + finPhase.ToString() + "_DejaDemoli_" + s.Name + s.SheetNumber + "_Filtromatisation", catids);
                ParameterFilterElement f5 = f_PasConstruit(doc, debutPhase, finPhase, c0, c1, d0, d1, "GLS_PHA_" + debutPhase.ToString() + "_" + finPhase.ToString() + "_PasEncoreConstruit_" + s.Name + s.SheetNumber + "_Filtromatisation", catids);
                //t.Commit();
                #endregion

                #region adding filters to views
                foreach (ElementId viewId in s.GetAllPlacedViews())
                {
                    ...
                    overrideFilterGraphics(f0, v, filledPattern, c_Demoli_Fill, l_Demoli_Fill, l_Demoli_Cut, l_Demoli_projectionLineWeight, l_Demoli_cutLineWeight, filledPattern, c_Demoli_Fill, l_Demoli_surfaceTransp, filledPattern, l_Demoli_Fill, filledPattern, c_Demoli_Fill); ;
                    overrideFilterGraphics(f2, v, filledPattern, c_Nouveau_Fill, l_Nouveau_Fill, l_Nouveau_Cut, l_Nouveau_projectionLineWeight, l_Nouveau_cutLineWeight, null, null, -1, null, null, filledPattern, c_Nouveau_Cut);
                    overrideFilterGraphics(f3, v, filledPattern, c_Existant_Proj, l_Existant_projectionLineColor, l_Existant_cutLineColor, l_Existant_projectionLineWeight, l_Existant_cutLineWeight, filledPattern, l_Existant_SurfColor, -1, null, null, filledPattern, c_Existant_Cut);
                    v.SetFilterVisibility(f4.Id, false);
                    v.SetFilterVisibility(f5.Id, false);

                    s.GetParameters("GLS-PHA_Désignation")[0].Set("Motomatisey");
                }
                #endregion
            }
            #endregion
        ...
        public void timeMachine(Document doc, List<ElementId> catids)
        {
            ...
            Application.Run(timeMachine);
            int debutPhase = timeMachine.minDateOut;
            int finPhase = timeMachine.maxDateOut;


            if (timeMachine.onEffaceLesFiltres)
            {
...
namespace BH_TimeMachineForm
{
    ...
    public partial class Form1 : System.Windows.Forms.Form
    {
        ...
        private void trackBar1_Scroll(object sender, EventArgs e)
        {
            ...
            #region old
            /*
            int dateNumber = trackBar1.Value;
            int year = 2000 + (dateNumber / 10000);
            int month = dateNumber / 100 % 100;
            int day = dateNumber % 100;
            if ((month <= 12 && month != 0 && day <= DateTime.DaysInMonth(year, month) && day != 0))
            {
...
namespace BH_TimeMachineForm
{
    partial class Form1
    {
        ...
        private void InitializeComponent()
        {
            ...
            //this.trackBar1.TickColor = System.Drawing.Color.FromArgb(((int)(((byte)(192)))), ((int)(((byte)(192)))), ((int)(((byte)(0)))));
            ...
            this.button1.Text = "Générer les filtres";
            ...
            this.button2.Text = "Effacer les filtre";
            ...
            this.label3.Text = "Version 14.09.2020";
            ...
            this.label10.Location = new System.Drawing.Point(250, 292);
